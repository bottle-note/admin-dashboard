...
식약처에서 수집·정제한 주류 수입 신고 데이터를 운영자가 어드민에서 조회할 수 있도록 `수입 정보` 영역을 추가한다.
...
수입 신고 목록·상세를 P0으로 먼저 제공하고, 수입사 목록·상세를 P1로 연결한다. 이번 phase는 조회 전용이며 수집 실행, 정제 상태 변경, 매칭 확정, 수입사 연결과 같은 데이터 변경 기능은 포함하지 않는다.
...
식약처에서 수집한 주류 신고 데이터 한 건이다. 제품명, 용량, 도수, 제조사, 국가, 수입사, 정제 결과와 매칭 정보를 포함한다. `id`는 어드민 데이터의 식별자이고 `rcno`는 식약처 수입신고번호로 사용한다.

### 정규화

수집한 원문에서 제품명, 용량, 도수, 숙성 연수, 빈티지, 배치 등의 구조화된 값을 추출하는 과정이다. 원문과 정제 결과를 함께 보여주고, `normalizationReasons`와 `unparsedFragments`를 통해 운영자가 판단 근거와 미처리 내용을 확인할 수 있어야 한다.
...
수입신고번호와 수입사를 연결한 수집 원장이다. 식약처 원문상의 수입사명, 연결 방식, 출처 URL과 관측 시각을 보존한다. 운영자가 현재 수입사 연결을 변경해도 이 원장은 바뀌지 않으므로 현재 상태가 아니라 출처·감사 정보로 취급한다.

### 수입사

식약처에서 확인된 수입 업체다. 업체 기본 정보와 운영 관리 상태를 갖는다. 수입사 상세에서는 `importerId`로 수입 신고 목록을 다시 조회해 해당 업체를 통해 들어온 술을 함께 보여준다.

## Screen Map

| Priority | Screen | Route | Purpose |
| --- | --- | --- | --- |
| P0 | 수입 신고 목록 | `/mfds/declarations` | 수집된 술을 검색·필터하고 검토할 신고를 찾는다. |
| P0 | 수입 신고 상세 | `/mfds/declarations/:declarationId` | 원본·정제 결과, 수입사 근거, 매칭 상태를 한곳에서 확인한다. |
| P1 | 수입사 목록 | `/mfds/importers` | 수입사를 검색하고 업체별 수입 이력으로 진입한다. |
| P1 | 수입사 상세 | `/mfds/importers/:importerId` | 업체 정보와 해당 업체에 연결된 수입 신고 목록을 확인한다. |

사이드바에는 `ROOT_ADMIN` 전용 최상위 그룹 `수입 정보`를 추가하고, `수입 신고 목록`과 `수입사 목록`을 하위 메뉴로 제공한다. 매칭 후보와 RCNO 연결 근거는 독립 메뉴로 만들지 않고 수입 신고 상세의 보조 영역으로 제공한다.

## Admin Workflow

1. 관리자는 `수입 정보 > 수입 신고 목록`으로 진입한다.
2. 제품명·수입신고번호 검색, 정규화 상태, 매칭 여부, 수입사 등의 조건으로 신고를 찾는다.
3. 목록 행을 선택해 수입 신고 상세로 이동한다.
4. 상세에서 원본과 정제 결과, 확인 사유, 수입사 연결과 출처, 현재 매칭 결과와 후보를 확인한다.
...
1. 신고 기본 정보
   - RCNO, 원본·정제 제품명, 카테고리, 제조사, 제조·수출 국가
2. 제품 규격
   - 원본·정제 용량, 단위 용량, 포장 수량, 원본·정제 도수
   - 숙성 연수, 빈티지, 에디션, 캐스크 번호, 배치 번호
3. 정제 및 검토 상태
   - 정규화 상태, 정규화 사유, 파싱하지 못한 원문
   - 정제·검토 시각, 검토자와 메모
4. 수입사
   - 현재 연결 수입사와 연결 방식·시각
   - 수입사 상세 이동
...
상세에서 데이터 변경 버튼은 제공하지 않는다. 후보나 현재 연결 값이 없으면 오류처럼 보이지 않도록 `연결된 정보 없음`, `저장된 후보 없음`으로 표시한다.
...
- 수입 신고 이력:
  ...
  - 수입 신고 목록과 같은 핵심 열과 커서 페이징을 제공한다.
  - 신고 행 선택 시 해당 수입 신고 상세로 이동한다.
...
- [ ] `ROOT_ADMIN`은 사이드바의 `수입 정보`에서 수입 신고 목록과 수입사 목록으로 진입할 수 있다.
- [ ] 수입 신고 목록은 검색·필터·커서 페이징 상태를 URL 파라미터로 유지한다.
- [ ] 수입 신고 목록에서 로딩, 빈 결과, 검색 결과 없음, 오류 상태가 구분된다.
- [ ] 수입 신고 상세에서 원본·정제 결과와 정규화 사유를 확인할 수 있다.
- [ ] 수입 신고 상세에서 현재 수입사 연결과 RCNO 연결 근거를 구분해 확인할 수 있다.
- [ ] 수입 신고 상세에서 확정된 매칭과 저장된 후보를 구분해 확인할 수 있다.
