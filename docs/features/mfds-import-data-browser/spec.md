...
`식약처 데이터 관리 > 수입사 관리`에서 수입사를 조회하고, 상세·등록·수정·삭제를 처리한다.
...
- 수정 화면에서는 운영용 업체명, 공개 설명, 관리자 메모, 관리 상태만 변경한다. 식약처 공식 정보는 수입 신고의 식별·연결 기준값이므로 읽기 전용이다.
- 상세 화면에서 수입사를 삭제할 수 있다. 연결된 수입 신고 또는 RCNO 연결 근거가 있으면 API가 삭제를 거절한다.

## Current Delivery: 신고 데이터 검토

신고 상세에서 운영자가 현재 정규화 결과를 검토하고 판정한다. 이번 단계에서는 정규화 값 자체를 수정하지 않는다.

- 제목의 정규화 상태 옆 `검토` 버튼은 `REVIEW_REQUIRED`, `UNPARSED`, `PARTIAL`, `NORMALIZED` 상태에서만 노출한다. `PENDING`, `STALE`은 수집·정규화 파이프라인 상태이므로 운영자가 직접 바꾸지 않는다.
- `신고 데이터 검토` 모달은 제품명, RCNO, 현재 상태를 보여주고 아래 판단만 제공한다.
  - 문제 없음 → `NORMALIZED`
  - 일부 정보 확인 불가 → `PARTIAL`, 메모 필수
  - 추가 검토 필요 → `REVIEW_REQUIRED`, 메모 필수
- `PATCH /v1/mfds/declarations/{declarationId}/normalization-status`에는 선택한 상태, 로그인 관리자 이메일, 검토 메모를 보낸다. 저장 후 모달을 닫고 상세·목록 상태를 갱신한다.
- 검토 이력이 있으면 정규화 완료 상태에서도 처리 상태 패널에 검토자·시각·메모를 남긴다.
- 이후 정규화 필드 직접 수정 기능은 같은 모달 안에서 확장하며, 원문과 운영자 보정값·보정 사유를 별도로 다룬다.
...
상세에서는 정규화 값 수정 버튼을 제공하지 않는다. `검토` 버튼으로 상태와 검토 메모만 변경한다. 후보나 현재 연결 값이 없으면 오류처럼 보이지 않도록 `연결된 정보 없음`, `저장된 연결 후보 없음`으로 표시한다.
...
- 목록 상단에서 수입사를 등록할 수 있다. 삭제는 상세 화면에서만 제공한다.
...
- 업체명, 설명, 관리자 메모, 관리 상태를 수정할 수 있다. 식약처 공식 정보는 읽기 전용이다. 삭제는 상세 화면의 확인 모달을 거쳐 수행한다.
...
- `PATCH /v1/mfds/declarations/{declarationId}/normalization-status`
  - Request: `normalizationStatus`, `reviewedBy`, `reviewNote`
  - Response data: `AdminResultResponse`
...
- [ ] 운영자는 신고 상세의 검토 모달에서 문제 없음·일부 정보 확인 불가·추가 검토 필요를 기록할 수 있다.
