...
- 현재 목록 응답의 `baseProductNameKo`, `baseProductNameEn`만 사용한다. 상세의 SKU 표시명은 이번 POC에서 조회하지 않는다.
- 제품명 쌍의 앞뒤 공백을 제거해 완전히 같은 `(baseProductNameKo, baseProductNameEn)` 행은 하나로 통합한다. 이름 둘 다 없는 행은 파일에 넣지 않는다.
- 내려받는 파일은 등록 가능한 완성 파일이 아니라 `한글 이름`, `영문 이름`만 채운 **등록 초안**이다. 나머지 필수 칼럼은 빈 값으로 둔다.
...
- [ ] 내려받은 초안의 `알코올 데이터` 시트에는 중복 제거된 기본 한글·영문 제품명만 채워지고, 기존 템플릿의 시트·헤더·설명 구조는 유지된다.
