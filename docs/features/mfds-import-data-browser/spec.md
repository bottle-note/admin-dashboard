...
4. 수입사 연결
   - 현재 연결, 연결 상태와 처리 시각만 표로 표시한다.
   - 별도 상세 드로어를 제공하지 않고, 현재 연결된 수입사명을 상세 경로로 이동하는 링크로 표시한다.
   - 연결 방식과 RCNO 연결 근거는 중복 정보로 판단해 노출하지 않는다.
   - 연결된 수입사명은 `/mfds/importers/:importerId` 상세 경로로 연결한다.
   - 연결된 수입사가 없으면 안내만 표시한다. 수입사 검색과 수동 연결은 후속 단계에서 제공한다.
5. 연관 데이터 연결
