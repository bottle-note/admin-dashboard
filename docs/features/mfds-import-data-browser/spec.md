...
## Proposed Delivery: 신규 위스키 등록 초안 다운로드 (POC)

수입 신고 목록을 보틀노트 DB 커버리지의 입력원으로 사용한다. 이번 POC는 신규 위스키를 만들거나 연결하지 않는다. 운영자가 목록에 적용한 현재 필터 결과의 수집 제품명을 기존 알코올 XLSX 템플릿에 채워 내려받게 한다.

운영자는 내려받은 파일에 수기로 정보를 채우거나, 별도 AI 작업으로 값을 보완한 뒤 현재의 알코올 엑셀 검증·등록 흐름에 올린다. 필터는 운영자가 판단할 수 있는 도구이고, POC는 그 선택 결과를 바꾸지 않는다.

- 대상은 현재 커서 페이지의 최대 100건이다. 다음 페이지까지 자동 수집하지 않는다.
- 내보내기 버튼은 현재 적용한 검색어, 수입사, 정규화 상태, 보틀노트 연결 여부, 연결 판정을 그대로 따른다. 현재 목록이 비어 있거나 로딩·오류 상태면 실행할 수 없다.
- 필터 영역의 우측에 primary `Excel 등록 초안 다운로드` 버튼을 둔다. 현재 적용 조건은 기존 검색·필터 컨트롤의 선택 상태로 확인한다.
- 현재 목록 응답의 SKU 표시명을 우선 사용하고 값이 없으면 기본 제품명을 사용한다. `abvPercent`, `volumeMl`도 값이 있는 행만 해당 셀에 채운다.
- 이름의 앞뒤 공백을 제거하고 `(한글 이름, 영문 이름, 도수, 용량)`이 모두 같은 행은 하나로 통합한다. 이름 둘 다 없는 행은 파일에 넣지 않는다.
- 내려받는 파일은 등록 가능한 완성 파일이 아니라 목록에서 확인된 `한글 이름`, `영문 이름`, `도수`, `용량`만 채운 **등록 초안**이다. 목록에 없거나 값이 없는 칼럼은 빈 값으로 둔다.
- 현재 알코올 템플릿 API가 반환한 XLSX를 브라우저에서 열어 `알코올 데이터` 시트의 3행부터 채운 뒤 다시 다운로드한다. 웹 워커는 사용하지 않는다.
- 템플릿의 참조 시트·시트 순서·1~2행 헤더와 설명을 유지해야 한다. POC 결과 파일은 실제 알코올 엑셀 검증 API로 검증해 호환성을 확인한다.
...
- `GET /v1/alcohols/excel/template`
  - 기존 알코올 벌크 등록의 고정 XLSX 템플릿 바이너리를 반환한다.
  - 시트 순서와 입력 시트의 1행 헤더, 2행 설명은 검증 시 변경할 수 없다.
- `POST /v1/alcohols/excel/validate`
  - 완성된 알코올 XLSX를 저장 없이 파싱·검증한다.
  - POC가 만든 등록 초안은 필수 필드가 비어 있어 유효하지 않은 것이 정상이며, AI 또는 운영자의 보완 후 사용하는 검증 단계다.
...
- [ ] 운영자가 현재 적용한 목록 필터 결과에서 최대 100건을 등록 초안 XLSX로 내려받을 수 있다.
- [ ] 내려받은 초안의 `알코올 데이터` 시트에는 중복 제거된 SKU 표시명(없으면 기본 제품명), 도수, 용량 중 목록에 있는 값만 채워지고, 기존 템플릿의 시트·헤더·설명 구조는 유지된다.
- [ ] 대상 행이 없거나 목록이 로딩·오류 상태면 다운로드 버튼은 실행할 수 없다.
...
5. 신규 위스키 등록 초안 다운로드 (POC)
   - 목록 필터 조건 확인, 템플릿 다운로드·브라우저 XLSX 채우기·파일 다운로드, 현재 페이지 내 이름 중복 제거, 실제 템플릿 검증, E2E를 함께 제공한다.
...
- 현재 목록 필터 결과의 현재 페이지 기준 등록 초안 XLSX 다운로드
...
- 모든 커서 페이지를 한 번에 내보내는 서버 측 엑셀 생성 API
- 신고 데이터에서 알코올 등록 필드를 AI로 보완하거나, 완성된 파일을 자동 등록하는 기능
...
- POC에서 생성 파일의 템플릿 보존이 확인되면, 중복 묶음의 원장 근거와 AI 보완 결과를 별도 작업 큐로 관리한다.
